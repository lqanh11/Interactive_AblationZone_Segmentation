...
class ISTrainer(object):
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            # print(batch_data['instances'].shape, torch.max(batch_data['instances']), torch.min(batch_data['instances']))

            # print(output['instances'].shape, torch.max(output['instances']), torch.min(output['instances']))
            # print(output['instances_aux'].shape, torch.max(output['instances_aux']), torch.min(output['instances_aux']))
