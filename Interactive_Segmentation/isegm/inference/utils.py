...
from isegm.data.datasets import GrabCutDataset, BerkeleyDataset, DavisDataset, SBDEvaluationDataset, PascalVocDataset,RetinalDataset, AblationZone, AblationZone_Test
...
def get_dataset(dataset_name, cfg):
    ...
    elif dataset_name == 'ABLATION_TRAIN':
        dataset = AblationZone(cfg.ABLATIONZONE_TRAIN_PATH)
    elif dataset_name == 'ABLATION_VAL':
        dataset = AblationZone(cfg.ABLATIONZONE_VAL_PATH)
    elif dataset_name == 'ABLATION_TEST':
        dataset = AblationZone_Test(cfg.ABLATIONZONE_TEST_PATH)
    elif dataset_name == 'RETINAL_VAL':
        dataset = RetinalDataset(cfg.RETINAL_VAL_PATH)
    elif dataset_name == 'IDRID_VAL':
        dataset = RetinalDataset(cfg.IDRID_VAL_PATH)
    elif dataset_name == 'DDR_VAL':
        dataset = RetinalDataset(cfg.DDR_VAL_PATH)
    elif dataset_name == 'HOSPITAL_VAL':
        dataset = RetinalDataset(cfg.HOSPITAL_VAL_PATH)
    else:
        dataset = None
...
def get_iou(gt_mask, pred_mask, ignore_label=-1):
    ## this function for dice calculation but change name
    ignore_gt_mask_inv = gt_mask != ignore_label
    obj_gt_mask = gt_mask == 1
...
def get_dsc(gt_mask, pred_mask, ignore_label=-1):
    ignore_gt_mask_inv = gt_mask != ignore_label
    obj_gt_mask = gt_mask == 1
...
def get_results_table(noc_list, over_max_list, brs_type, dataset_name, mean_spc, elapsed_time,
    ...
    table_header = (f'|{"BRS Type":^13}|{"Dataset":^11}|'
                    f'{"NoC@45%":^9}|{"NoC@50%":^9}|{"NoC@55%":^9}|{"NoC@60%":^9}|{"NoC@65%":^9}|{"NoC@70%":^9}|{"NoC@75%":^9}|{"NoC@80%":^9}|'
                    f'{">="+str(n_clicks)+"@70%":^9}|{">="+str(n_clicks)+"@80%":^9}|'
                    f'{"SPC,s":^7}|{"Time":^9}|')
    ...
    table_row += f'{noc_list[5]:^9.2f}|' if len(noc_list) > 5 else f'{"?":^9}|'
    table_row += f'{noc_list[6]:^9.2f}|' if len(noc_list) > 6 else f'{"?":^9}|'
    table_row += f'{noc_list[7]:^9.2f}|' if len(noc_list) > 7 else f'{"?":^9}|'
    table_row += f'{over_max_list[5]:^9}|' if len(noc_list) > 1 else f'{"?":^9}|'
    table_row += f'{over_max_list[7]:^9}|' if len(noc_list) > 2 else f'{"?":^9}|'
    print(len(over_max_list))
